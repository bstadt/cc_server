...
def install_post_commit_hook(repo_path: Path) -> None:
    """
    Install post-commit hook to sync versioned authz to conf/authz.
    
    This ensures that when users commit authz changes via claudeconnect sync,
    Apache access control is immediately updated.
    """
    hook_content = """#!/bin/bash
REPOS="$1"
REV="$2"

# Sync the versioned authz file to conf/authz for Apache
# This ensures access control changes take effect immediately
if svnlook tree "$REPOS" -r "$REV" authz >/dev/null 2>&1; then
    svnlook cat "$REPOS" authz > "$REPOS/conf/authz"
fi
"""
    ...
    # Write hook file
    subprocess.run(
        ["sudo", "tee", str(hook_path)],
        input=hook_content.encode(),
        stdout=subprocess.DEVNULL,
        check=True,
    )
    
    # Make executable and set ownership
    subprocess.run(["sudo", "chmod", "+x", str(hook_path)], check=True)
    subprocess.run(["sudo", "chown", "www-data:www-data", str(hook_path)], check=True)
...
def create_svn_repo(repo_name: str, email: str) -> tuple[bool, str]:
    ...
    try:
        ...
        # Install post-commit hook to sync authz changes
